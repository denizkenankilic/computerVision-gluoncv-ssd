...
dataset = gcv.data.RecordFileDetection('AOI_02_tile_512.rec')
classes = ['car']  # only one foreground class here
...
def get_dataloader(net, train_dataset, data_shape_1, data_shape_2, batch_size, num_workers):
    from gluoncv.data.batchify import Tuple, Stack, Pad
    from gluoncv.data.transforms.presets.ssd import SSDDefaultTrainTransform
    width, height = data_shape_1, data_shape_2
...
train_data = get_dataloader(net, dataset, 512, 512, 8, 0)
#############################################################################################
...
net.save_parameters('ssd_512_resnet50_v1_AOI_02_tile_512.params')
...
net.load_parameters('ssd_512_resnet50_v1_AOI_02_tile_512.params')

testImagesPath = r'AOI_02\test_for_AOI02_tile_512'
imageNames = os.listdir(testImagesPath)
imageNames = [os.path.join(testImagesPath, images) for images in imageNames]
cidResultFile = 'cidResults.pickle'
f = open(cidResultFile,'w')
f.close()
cidResults = {}
scoreResultFile = 'scoreResults.pickle'
g = open(scoreResultFile,'w')
g.close()
scoreResults = {}
bboxResultFile = 'bboxResults.pickle'
h = open(bboxResultFile,'w')
h.close()
bboxResults = {}


for i in range(len(imageNames)):
     x, test_image = gcv.data.transforms.presets.ssd.load_test(imageNames[i], 512)
     cid, score, bbox = net(x)
     try:
         f = open(cidResultFile,'wb')
         cidResults['cidResults'+ str(i)]= cid
         pickle.dump(cidResults, f, pickle.HIGHEST_PROTOCOL)
         f.close()
     except Exception as e:
         print('Unable to save data to', cidResultFile, ':', e)
     try:
         g = open(scoreResultFile,'wb')
         scoreResults['scoreResults'+ str(i)]= score
         pickle.dump(scoreResults, g, pickle.HIGHEST_PROTOCOL)
         g.close()
     except Exception as e:
         print('Unable to save data to', scoreResultFile, ':', e)
     try:
         h = open(bboxResultFile,'wb')
         bboxResults['bboxResults'+ str(i)]= bbox
         pickle.dump(bboxResults, h, pickle.HIGHEST_PROTOCOL)
         h.close()
     except Exception as e:
         print('Unable to save data to', bboxResultFile, ':', e)
     ax = viz.plot_bbox(test_image, bbox[0], score[0], cid[0], class_names=classes)
